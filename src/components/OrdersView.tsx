...
export default function OrdersView({
  ...
  const handleCreateOrder = (e: React.FormEvent) => {
    ...
    if (!selectedCustomerId) {
      alert('Vui lòng tìm và chọn một khách hàng từ danh sách gợi ý!');
      return;
    }
    if (!selectedCourseId) {
      alert('Vui lòng chọn sản phẩm / khóa học!');
      return;
    }
    ...
    if (!cust || !course) {
      alert('Không tìm thấy thông tin khách hàng hoặc khóa học đã chọn.');
      return;
    }
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {onDeleteOrder && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-[10px] font-bold transition cursor-pointer"
                          >
                            Xóa
                          </button>
                        )}
